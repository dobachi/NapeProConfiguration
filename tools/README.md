...
| 常時ジェスチャー/スクロール・スリープ・ポーリング | ✅ | ❌（SET コマンド未確定）|
| バッテリー | ✅（参照のみ）| — |

回転角は値 0〜7（×45 = 度）。詳細は [`../docs/rotation-angle-investigation.md`](../docs/rotation-angle-investigation.md)。

## 既知の制限

- **常時モード／スリープタイマー／ポーリングレートはインポートで復元されない**（GET のみ判明、
  SET コマンド未確定）。エクスポートには `deviceInfo` として記録されるので、復元後は Launcher で手動設定する。
