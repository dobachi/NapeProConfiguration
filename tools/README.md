...
- **トラックボールの回転角度（向き設定）はインポートで復元されない。** 現状の対象はキーマップ・
  エンコーダ・DPI・combos/gesture/tapholds/profile。向き／回転角度に相当する設定
  （`KC_USER_CMD_NAPE_GET_LAYER_ORI 0xa7,0x38` 等）は未対応のため、復元後は Launcher で手動設定する。
