...
> ℹ️ 回転角（向き設定）は v1.1 以降のエクスポート/インポートで対応済み。
> この設定ファイルは旧版（v1.0 相当）で回転角を含まないため、再エクスポートで取り込める。
...
- **回転角（向き設定）は対応済み**（エクスポート v1.1 以降）。この設定ファイルは v1.0 相当のため未収録 →
  再エクスポートで取り込める。なおこのテーマでは Launcher 上で Layer 2 のみ 90° に設定されている。
- 常時モード／スリープ／ポーリングはエクスポートのみ（SET コマンド未確定でインポート未対応）。
