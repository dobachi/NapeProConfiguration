...
| 項目 | 値 |
|------|----|
| コンフィグ | [`nape-pro-settings-2026-06-29.json`](nape-pro-settings-2026-06-29.json)（スキーマ v1.3 / 全項目収録）|
| エクスポート日 | 2026-06-29 |
...
> ℹ️ スキーマ v1.3 で全項目収録（回転角・combos・tap-hold・gesture・常時モード・スリープ・ポーリング）。
> インポートで復元できる（バッテリーは参照のみ）。各レイヤ図に回転角も表示。

## 同時押し（combos）

すべてレイヤ 0 に登録（col↔ボタン: 0=03,1=04,2=01,3=02,4=M1,5=M2）。

| トリガー | 出力(tap) | 長押し(held) |
|----------|-----------|--------------|
| 03 ＋ 01 | Print Screen | — |
| 01 ＋ M2 | 独自 0x5201 | 独自 0x5201 |
| 02 ＋ M2 | 独自 0x5203 | 独自 0x5201 |

ボールジェスチャ: 左=0x950 / 右=0x94F（独自コード）。tap-hold は未設定。

> デバイス概要・物理レイアウト・OctaShift・**レイヤ番号のズレ（表示 vs 内部）** は
> [`../../docs/nape-pro-device.md`](../../docs/nape-pro-device.md) を参照。本テーマの「横置き／横置き設定用／
> プレゼン縦持ち」は、各レイヤに紐づく回転角（向き）を、レイヤ／向き切替で能動的に切り替えて使い分ける。
...
> 予備ボタン（Col6）は全レイヤで `モード/レイヤ (0x522B)`。レイヤ／向きの切替に関わると推定。
